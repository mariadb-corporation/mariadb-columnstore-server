...
Item::Item(THD *thd):
  m_thd(thd), is_expensive_cache(-1), rsize(0), name(null_clex_str), orig_name(0),
...
void Item_ident::print(String *str, enum_query_type query_type)
{
  ...
  if (query_type == QT_INFINIDB || query_type == QT_INFINIDB_NO_QUOTE || query_type == QT_INFINIDB_DERIVED)
  {
  	...
  	{
  		if (cached_table->db.length > 0)
	  	{
            str->append(cached_table->db.str, cached_table->db.length);
            str->append('.');
        }
        ...
        {
            str->append(cached_table->referencing_view->alias.str, cached_table->referencing_view->alias.length);
            str->append("_");
        }
        str->append(cached_table->alias.str, cached_table->alias.length);
        str->append('.');
    }
  	else
  	{
